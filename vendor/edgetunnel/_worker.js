/*!
 * vendored from https://github.com/cmliu/edgetunnel (GPL-2.0)
 * pinned commit: fb3212257e3527447d7368010b378f7e449444b4
 * upstream Version: 2026-08-11 14:45:22
 * vendor 更新方式: 替换本文件以下全部内容, 并更新上面三行
 */
...
async function SS派生主密钥(passwordText, keyLen) {
	...
	const deriveTask = (async () => {
		...
		while (result.byteLength < keyLen) {
			...
			prev = new Uint8Array(await md5Polyfill(input));
...
async function turnConnect(proxy, targetHost, targetPort, TCP连接) {
	...
	try {
		...
		if (
			...
			integrityKey = new Uint8Array(await md5Polyfill(textEncoder.encode(`${proxy.username}:${realm}:${proxy.password}`)));
			authAttributes = [
...
// RFC 1321 MD5 — verified against Node crypto via 7+ test vectors
async function md5Polyfill(input) {
  let msg = input instanceof Uint8Array ? input : new TextEncoder().encode(input);
  const ml = msg.length;
  ...
  const OP = (fn, a, b, c, d, k, s, T) => add(b, rol(add(add(add(a, fn(b, c, d)), k), T), s));
  ...
  for (let i = 0; i < padded.length; i += 64) {
    ...
    // Round 1
    a = OP(F, a, b, c, d, x[ 0],  7, 0xd76aa478);
    d = OP(F, d, a, b, c, x[ 1], 12, 0xe8c7b756);
    c = OP(F, c, d, a, b, x[ 2], 17, 0x242070db);
    b = OP(F, b, c, d, a, x[ 3], 22, 0xc1bdceee);
    a = OP(F, a, b, c, d, x[ 4],  7, 0xf57c0faf);
    d = OP(F, d, a, b, c, x[ 5], 12, 0x4787c62a);
    c = OP(F, c, d, a, b, x[ 6], 17, 0xa8304613);
    b = OP(F, b, c, d, a, x[ 7], 22, 0xfd469501);
    a = OP(F, a, b, c, d, x[ 8],  7, 0x698098d8);
    d = OP(F, d, a, b, c, x[ 9], 12, 0x8b44f7af);
    c = OP(F, c, d, a, b, x[10], 17, 0xffff5bb1);
    b = OP(F, b, c, d, a, x[11], 22, 0x895cd7be);
    a = OP(F, a, b, c, d, x[12],  7, 0x6b901122);
    d = OP(F, d, a, b, c, x[13], 12, 0xfd987193);
    c = OP(F, c, d, a, b, x[14], 17, 0xa679438e);
    b = OP(F, b, c, d, a, x[15], 22, 0x49b40821);

    // Round 2
    a = OP(G, a, b, c, d, x[ 1],  5, 0xf61e2562);
    d = OP(G, d, a, b, c, x[ 6],  9, 0xc040b340);
    c = OP(G, c, d, a, b, x[11], 14, 0x265e5a51);
    b = OP(G, b, c, d, a, x[ 0], 20, 0xe9b6c7aa);
    a = OP(G, a, b, c, d, x[ 5],  5, 0xd62f105d);
    d = OP(G, d, a, b, c, x[10],  9, 0x02441453);
    c = OP(G, c, d, a, b, x[15], 14, 0xd8a1e681);
    b = OP(G, b, c, d, a, x[ 4], 20, 0xe7d3fbc8);
    a = OP(G, a, b, c, d, x[ 9],  5, 0x21e1cde6);
    d = OP(G, d, a, b, c, x[14],  9, 0xc33707d6);
    c = OP(G, c, d, a, b, x[ 3], 14, 0xf4d50d87);
    b = OP(G, b, c, d, a, x[ 8], 20, 0x455a14ed);
    a = OP(G, a, b, c, d, x[13],  5, 0xa9e3e905);
    d = OP(G, d, a, b, c, x[ 2],  9, 0xfcefa3f8);
    c = OP(G, c, d, a, b, x[ 7], 14, 0x676f02d9);
    b = OP(G, b, c, d, a, x[12], 20, 0x8d2a4c8a);

    // Round 3
    a = OP(H, a, b, c, d, x[ 5],  4, 0xfffa3942);
    d = OP(H, d, a, b, c, x[ 8], 11, 0x8771f681);
    c = OP(H, c, d, a, b, x[11], 16, 0x6d9d6122);
    b = OP(H, b, c, d, a, x[14], 23, 0xfde5380c);
    a = OP(H, a, b, c, d, x[ 1],  4, 0xa4beea44);
    d = OP(H, d, a, b, c, x[ 4], 11, 0x4bdecfa9);
    c = OP(H, c, d, a, b, x[ 7], 16, 0xf6bb4b60);
    b = OP(H, b, c, d, a, x[10], 23, 0xbebfbc70);
    a = OP(H, a, b, c, d, x[13],  4, 0x289b7ec6);
    d = OP(H, d, a, b, c, x[ 0], 11, 0xeaa127fa);
    c = OP(H, c, d, a, b, x[ 3], 16, 0xd4ef3085);
    b = OP(H, b, c, d, a, x[ 6], 23, 0x04881d05);
    a = OP(H, a, b, c, d, x[ 9],  4, 0xd9d4d039);
    d = OP(H, d, a, b, c, x[12], 11, 0xe6db99e5);
    c = OP(H, c, d, a, b, x[15], 16, 0x1fa27cf8);
    b = OP(H, b, c, d, a, x[ 2], 23, 0xc4ac5665);

    // Round 4
    a = OP(I, a, b, c, d, x[ 0],  6, 0xf4292244);
    d = OP(I, d, a, b, c, x[ 7], 10, 0x432aff97);
    c = OP(I, c, d, a, b, x[14], 15, 0xab9423a7);
    b = OP(I, b, c, d, a, x[ 5], 21, 0xfc93a039);
    a = OP(I, a, b, c, d, x[12],  6, 0x655b59c3);
    d = OP(I, d, a, b, c, x[ 3], 10, 0x8f0ccc92);
    c = OP(I, c, d, a, b, x[10], 15, 0xffeff47d);
    b = OP(I, b, c, d, a, x[ 1], 21, 0x85845dd1);
    a = OP(I, a, b, c, d, x[ 8],  6, 0x6fa87e4f);
    d = OP(I, d, a, b, c, x[15], 10, 0xfe2ce6e0);
    c = OP(I, c, d, a, b, x[ 6], 15, 0xa3014314);
    b = OP(I, b, c, d, a, x[13], 21, 0x4e0811a1);
    a = OP(I, a, b, c, d, x[ 4],  6, 0xf7537e82);
    d = OP(I, d, a, b, c, x[11], 10, 0xbd3af235);
    c = OP(I, c, d, a, b, x[ 2], 15, 0x2ad7d2bb);
    b = OP(I, b, c, d, a, x[ 9], 21, 0xeb86d391);
...
async function MD5MD5(文本) {
	...
	const 第一次哈希 = await md5Polyfill(编码器.encode(文本));
	const 第一次哈希数组 = Array.from(new Uint8Array(第一次哈希));
	const 第一次十六进制 = 第一次哈希数组.map(字节 => 字节.toString(16).padStart(2, '0')).join('');

	const 第二次哈希 = await md5Polyfill(编码器.encode(第一次十六进制.slice(7, 27)));
